...
if x_axis and y_axis:
    ...
    malignant_df.plot.scatter(x=x_axis, y=y_axis, s=120, c="maroon", alpha=0.4, ax=scatter_ax, label="Malignant")
    benign_df.plot.scatter(x=x_axis, y=y_axis, s=120, c="green", alpha=0.4, ax=scatter_ax,
...
if bar_axis:
    ...
    sub_avg_breast_cancer_df.plot.bar(alpha=0.7, ax=bar_ax, title="Average Measurements per Tumor Type");
...
if hist_axis:
    ...
    sub_breast_cancer_df.plot.hist(bins=bins, alpha=0.5, ax=hist_ax, title="Distribution of Measurements");
else:
    hist_fig = plt.figure(figsize=(6,4))
